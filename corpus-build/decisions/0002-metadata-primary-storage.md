...
Corpus metadata is intended to be stored in a Git repository. Git supplies
history, snapshots, branching, merging, and transport; corpus-build must not
reimplement those facilities. This requirement does not make a Git commit hash
a domain identifier: ADR 0012 assigns UUIDs to artifact nodes and logical batch
transactions, while a commit hash identifies the metadata snapshot containing
their records. Commit creation remains an explicit repository operation rather
than hidden behaviour of `StorageBackend`.
...
- Metadata records can be reviewed and versioned with ordinary Git operations;
  the storage abstraction does not provide a second version-control system.
